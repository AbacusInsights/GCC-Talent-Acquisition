  ...
  const renderDisplayedText = (t) =>
    t.split("\n").map((line, i) => (
      <React.Fragment key={i}>
        {i === 2 ? <span className="text-[#0ea5e9]">{line}</span> : line}
        {i < t.split("\n").length - 1 && <br />}
      </React.Fragment>
    ));

  return (
    <section className="flex justify-center items-center min-h-[85vh] bg-[#e3e8f1ff] px-4 sm:px-6 md:px-10 lg:px-20 xl:px-28 overflow-hidden font-roboto py-10 md:py-14">
      <div className="w-full max-w-[1500px] grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        ...
        <div className="max-w-3xl flex flex-col items-start">
          ...
          <div className="min-h-[160px] sm:min-h-[200px] md:min-h-[250px] lg:min-h-[280px] mb-6">
            <h1 className="text-[1.9rem] sm:text-[2.4rem] md:text-[3rem] lg:text-[3.6rem] xl:text-[4rem] font-extrabold leading-[1.15] text-[#0f172a] whitespace-pre-wrap break-words">
        ...
        <div className="relative w-full h-[260px] sm:h-[350px] md:h-[420px] lg:h-[500px] xl:h-[560px] rounded-[20px] overflow-hidden shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_10px_10px_-5px_rgba(0,0,0,0.04)] bg-white">
          {images.map((src, index) => (
            <div key={src} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${index === currentImageIndex ? "opacity-100 z-10" : "opacity-0 z-0"}`}>
              <Image src={src} alt="" fill sizes="100vw" className="object-cover" priority={index === 0} />
            </div>
          ))}
        </div>
